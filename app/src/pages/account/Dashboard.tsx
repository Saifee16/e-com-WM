...
const Dashboard = () => {
  ...
  return <div className="space-y-8">
    ...
    <div className="grid lg:grid-cols-2 gap-8">
      ...
      <section className="bg-white rounded-2xl border border-gray-200 overflow-hidden"><div className="p-6 border-b flex items-center justify-between"><h3 className="text-lg font-bold">Wishlist</h3><Link to="/account/wishlist" className="text-blue-600 text-sm font-medium flex items-center gap-1">View All <ChevronRight className="w-4 h-4" /></Link></div>{wishlist.length === 0 ? <p className="p-8 text-center text-gray-500">Your wishlist is empty.</p> : <div className="divide-y divide-gray-100">{wishlist.map((item) => <Link key={item._id} to={`/products/${item._id}`} className="p-4 flex items-center gap-4 hover:bg-gray-50"><img src={item.images[0]} alt={item.name} className="w-14 h-14 bg-gray-100 rounded-lg object-cover" /><div className="flex-1 min-w-0"><p className="font-medium text-gray-900 line-clamp-1">{item.name}</p><p className="text-sm text-gray-500">{item.brand}</p></div><p className="font-medium text-blue-600">{formatPrice(item.price)}</p></Link>)}</div>}</section>
    </div>
