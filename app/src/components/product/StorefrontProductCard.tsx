  ...
  return (
    ...
    >
      <div className="relative">
        <ProductImage product={product} image={image} className="aspect-[1/0.84] p-3" />
      ...
      <div className="flex flex-1 flex-col p-3 sm:p-3.5">
        ...
        <h3 className="mt-2 line-clamp-2 min-h-12 text-[15px] font-bold leading-6 text-slate-950 transition group-hover:text-blue-700 sm:text-base">
          {product.name}
        </h3>
        ...
        <div className="mt-auto pt-3">
          ...
          {(isOutOfStock || onAddToCart) && (
            <div className={`mt-2 flex items-center gap-2 border-t border-slate-100 pt-2 ${isOutOfStock ? 'justify-between' : 'justify-end'}`}>
              {isOutOfStock && <span className="text-xs font-semibold text-slate-500">Out of stock</span>}
            {onAddToCart && (
              <AddToCartButton
                product={product}
                isOutOfStock={isOutOfStock}
...
const AddToCartButton = ({
  ...
  <button
    ...
    className={`inline-flex shrink-0 items-center justify-center gap-2 rounded-lg bg-blue-700 font-semibold text-white transition hover:bg-blue-800 active:translate-y-px disabled:cursor-not-allowed disabled:bg-slate-300 ${
      compact ? 'h-8 px-2.5 text-xs' : 'px-4 py-2.5 text-sm'
    }`}
